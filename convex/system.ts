...
import { assertProjectDeletable, deleteProjectTree } from "./lib/projects";

const validateInternalKey = (key: string) => {
  const internalKey = process.env.TORQ_AI_CONVEX_INTERNAL_KEY;

  if (!internalKey) {
    throw new Error("TORQ_AI_CONVEX_INTERNAL_KEY is not configured");
  }
  ...
  handler: async (ctx, args) => {
    ...
    for (const file of args.files) {
      const existing = existingFiles.find(
        (candidate) => candidate.name === file.name && candidate.type === "file",
      );

      if (existing || occupiedNames.has(file.name)) {
        results.push({
          name: file.name,
          fileId: existing?._id ?? "",
...
export const deleteProject = mutation({
  ...
  handler: async (ctx, args) => {
    validateInternalKey(args.internalKey);
    await assertProjectDeletable(ctx, args.projectId, args.ownerId);
    return await deleteProjectTree(ctx, args.projectId);
  },
});
